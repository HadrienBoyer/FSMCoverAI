    ...
    public class ZoneManager : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            foreach (var zone in Zones)
            {
                ...
                {
                    if (!TryGetPostPosition(post, out Vector3 postPosition))
                    {
                        continue;
                    }

                    if (zone.IsPointInZone(postPosition))
                    {
                        zone.Posts.Add(post);
                    }
                }
        ...
        private static bool TryGetPostPosition(IPost post, out Vector3 position)
        {
            position = default;
            ...
            if (post is UnityEngine.Object unityObject && unityObject == null)
            {
                return false;
            }
